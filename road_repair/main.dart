  ...
  Widget build(BuildContext context) {
    void segmentGenerator(){
      ...
      for (var x = x_old; x<20; x++) {
        ...
        var s = [start[0] + Random().nextDouble()*jump*(1/(x+1)),start[1] + Random().nextDouble()*jump*(1/(x+1))];
        var e = [(start[0] + end[0]*rand1)/(1+rand1),(start[1] + end[1]*rand2)/(1+rand2)];
        var loc = [(temp[0]+e[0])/2,(temp[1]+e[1])/2];
        Firestore.instance.collection('Segments').document('Route10001_Seg' + x.toString()).setData({
          'start':temp,
          'end':e,
          'location':loc,
          'flags':'none',
